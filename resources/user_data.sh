...
EOF
    ...
    for peer in "phone" "laptop" "docker"; do
        ...
        sudo wg set wg0 peer $PEER_PUB preshared-key $PEER_PSK persistent-keepalive 25 allowed-ips 0.0.0.0/0
...
EOF
    ...
    cat <<EOF | sudo tee /etc/bind/named.conf.logging
logging {
...
EOF
    sudo sed -i '/logging/d' /etc/bind/named.conf
    sudo sed -i '$ainclude "/etc/bind/named.conf.logging";' /etc/bind/named.conf
}

chmod() {
    for file in "/etc/bind/named.conf.local" "/etc/bind/named.conf.options"; do
        sudo chown root:bind "$file"
    done
}
